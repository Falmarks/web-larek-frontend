...
- **Методы**:
    - `get` - выполняет GET запрос на переданный ендпоинт
    - `post` - отправляет данные в JSON формате
...
Отвечает за хранение и логику работы с данными карточек товара.

- Конструктор принимает инстанс брокера событий
- **Поля**:
    - `_cards: ICard[]` - массив объектов карточек
    - `_preview: string | null` - id карточки для просмотра в модальном окне
    - `events: IEvents` - экземпляр EventEmitter
- **Методы**:
    - `getCard(cardId: string): ICard` - получение карточки по ID
    - `setCards(data: ICard[])` - установка массива карточек
...
- `card:selected` - изменение открываемой картинки карточки
- `card:previewClear` - очистка данных выбранной карточки

### События взаимодействия с интерфейсом

- `userEdit:open` - открытие формы редактирования пользователя
- `newCard:open` - открытие создания новой карточки
- `avatar:open` - открытие формы редактирования аватара
- `card:select` - выбор карточки для отображения
- `card:delete` - выбор карточки для удаления
- `card:like` - изменение состояния лайка
- `edit-profile:input` - изменение данных в форме профиля
- `edit-avatar:input` - изменение данных в форме аватара
- `new-place:input` - изменение данных в форме создания карточки
- `edit-profile:submit` - сохранение данных пользователя
- `edit-avatar:submit` - сохранение аватара пользователя
- `new-place:submit` - создание новой карточки
- `remove-card:submit` - подтверждение удаления карточки
- `edit-profile:validation` - валидация формы профиля
- `edit-avatar:validation` - валидация формы аватара
- `new-place:validation` - валидация формы создания карточки